...
public class MobileContactsApp {
    ...
    public static void insertContactService(String... contacts) throws Exception {
        boolean isInserted = false;
        if (contacts.length != 3 ) return;

        try {
            isInserted = insertContact(contacts[0], contacts[1], contacts[2]);
            if (!isInserted) {
    ...
    public static void updateContactService(String... contacts) throws Exception {
        boolean isUpdated = false;
        if (contacts.length != 3 ) return;

        try {
            isUpdated = insertContact(contacts[0], contacts[1], contacts[2]);
            if (!isUpdated) {
